...
const NavLink = () => {
  ...
  const pageName = Object.keys(pageWithSection).filter(key => pageWithSection[key]);
    ...
    <div className='flex flex-row space-x-10 tablet:space-x-6 phone:space-x-0 phone:flex-col phone:space-y-6'>
      {pageName.map((name: string, idx: number) => {
        return (
          <div
            key={idx}
            className='flex flex-col space-y-5 phone:space-y-4'
          >
            <Typography className='text-article text-opacity-60'>
              {name}
            </Typography>
            {pageLinks[idx].map((page: PageSection, idx: number) => {
              return (
                ...
                >
                  <a>
                    <Typography className='text-s text-article hover:text-opacity-60'>
                      {page.name}
                    </Typography>
                  </a>
                </Link>
